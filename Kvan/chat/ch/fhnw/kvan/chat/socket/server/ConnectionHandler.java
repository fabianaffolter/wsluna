 ...
 * client socket. It listens continuously for incoming data: If a new message
 * comes in, a new participant has announced himself or a participant left, this
 * information is forwarded to the ChatRoom. At the same time, every such
 * notification is temporarily stored here, until the ConnectionListener picks
 * it up.
 * 
...
public class ConnectionHandler extends Thread {
		...
		while ((s = in.readLine()) != null) {
			try {
				if (s.startsWith("add_participant=")) {
					...
					if (addParticipant(s)) {
						// Send current topic list and active participant list
						// to
						// new user
						println(chatRoom.getTopics());
						println(chatRoom.getParticipants());
						// temporarily store this notification to be sent to
						// other
						// active clients,
					...
					if (saveMessage(s)) {
						// temporarily store this message here to be sent to
						// other
						// active clients,
						// such as they receive this message as soon as it
						// arrived
						setMessage(s);
					}
					...
					if (addTopic(s)) {
						...
						// other
						// active clients,
						// such as they add the new topic to their local topic
						// list
						setMessage(s);
					}
	...
	private boolean removeTopic(String s) {
		...
		if (topicArray.length > 1 && !topicArray[1].equalsIgnoreCase("")) {
			try {
				...
				logger.error("ConnectionHandler IOException: could not remove topic "
						+ topicArray[1]);
			}
		}
		return success;
	...
	private void getMessages(String s) throws IOException {
		setMessage(chatRoom.getMessages(s));
	}
		...
		if (nameArray.length > 1) {
			...
			if (!clientName.equalsIgnoreCase("")) {
				try {
					...
					logger.error("ConnectionHandler IOException: could not remove participant "
							+ clientName);
				}
			}
		}
